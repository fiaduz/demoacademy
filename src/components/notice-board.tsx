...
export default function NoticeBoard() {
  ...
  return (
    <section className="bg-slate-950 border-y border-slate-800 py-4 md:py-6 px-4 md:px-6 overflow-hidden">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-stretch gap-4 md:gap-6">

        {/* Board Header */}
        <div className="bg-slate-900 border border-slate-800 text-white p-4 md:p-5 rounded-xl shadow-2xl md:w-64 shrink-0 flex flex-row md:flex-col justify-between items-center md:items-start relative overflow-hidden">
          ...
          <div className="min-w-0">
            ...
            <h3 className="text-lg md:text-xl font-serif font-bold text-slate-100">Notice Board</h3>
            <p className="text-xs text-slate-500 mt-0.5 hidden md:block leading-relaxed">
              Important updates, event deadlines, and operational listings.
            </p>
          </div>

          <div className="text-[10px] md:text-xs text-slate-400 md:pt-4 font-medium flex items-center gap-1 md:border-t md:border-slate-800/60 mt-0 md:mt-4 w-auto md:w-full shrink-0">
        ...
        >
          {displayNotices.map((notice, idx) => (
            <div
              ...
              className="h-14 flex items-center bg-slate-900/40 border border-slate-800/60 rounded-xl pl-4 pr-20 md:pr-24 py-3 hover:border-emerald-500/40 hover:bg-slate-900/90 transition-all group shrink-0 cursor-pointer relative"
            >
              <div className="flex items-center gap-2 md:gap-4 min-w-0 w-full">
                ...
                <p className="text-xs md:text-sm text-slate-300 font-medium truncate group-hover:text-emerald-400 transition-colors">
                  {notice.text}
                </p>
              </div>

              <div className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] md:text-xs text-slate-500 font-mono font-medium shrink-0 flex items-center gap-1 md:gap-2 bg-gradient-to-l from-slate-900/40 via-slate-900/40 to-transparent pl-2">
      ...
      {activeNotice && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md">
          ...
          <div className="relative w-full max-w-xl bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-5 md:p-8 overflow-hidden z-10 text-left flex flex-col max-h-[80vh] md:max-h-[85vh]">
            ...
            <div className="flex items-start justify-between gap-4 border-b border-slate-800 pb-4 shrink-0">
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-slate-500 font-mono">Posted: {activeNotice.date}, 2026</p>
              </div>
            ...
            <div className="overflow-y-auto my-4 md:my-5 pr-1 space-y-3 md:space-y-4 flex-1 scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
              <h4 className="text-base md:text-xl font-serif font-bold text-slate-100 leading-snug">
                {activeNotice.text}
              </h4>
              <div className="text-xs md:text-sm text-slate-400 leading-relaxed bg-slate-950/40 p-4 border border-slate-800/40 rounded-xl whitespace-pre-line">
